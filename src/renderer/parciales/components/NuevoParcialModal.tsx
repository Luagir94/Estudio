...
import { focusRingWithin, interactiveChip } from '../../shared/lib/interactive'
...
export function NuevoParcialModal({
  ...
  const gradeField = fields.bind('grade', errors.grade && translateValidationMessage(t, errors.grade.message))
          ...
          <form onSubmit={handleSubmit((values) => onSubmit({ ...values, subjectId }))} className="contents">
            <DialogBody>
              <Label>
                ...
                <Input
                  type="text"
                  placeholder={t('nuevoParcialModal.labelPlaceholder')}
                  {...register('label')}
                  {...labelField.control}
                />
              </Label>
              ...
              <div className="flex flex-col gap-2">
                <Label className="w-[200px]">
                  ...
                  <span
                    className={cn(
                      'flex h-10 w-full items-center gap-2 rounded-lg border border-input bg-background px-3',
                      focusRingWithin
                    )}
                  >
